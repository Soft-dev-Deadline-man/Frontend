...
export default function Location() {
  ...
  const searchData = async (e: any) => {
    if (search != "" || search != undefined) {
      const filter = tempBlogs?.filter((blog) =>
        blog.title
          .toLocaleLowerCase()
...
const filterByCatagory = async()=>{
  if(tempBlogs!=null){
    const filteredObjects = tempBlogs.filter((obj) => {
      return (
        ...
        (cetagories.history && obj.category === "ประวัติศาสตร์ วัฒนธรรม และศาสนา") ||
        (cetagories.museum && obj.category === "พิพิธภัณฑ์ และแหล่งเรียนรู้ทางธรรมชาติ") ||
  ...
  const multiFilter = async (e: any) => {
    const isSomeCatagoryTrue = Object.values(cetagories).some((value) => value === true);
   if(tempBlogs!=null){
    const filteredObjects = tempBlogs.filter((obj) => {
      return (
        (cetagories.breach && obj.category === "ชายหาดและทะเล") ||
    ...
    <main className="min-h-screen pt-24 relative max-w-screen-2xl mx-auto font-karnit px-4">
      ...
      <div className="flex w-full">
        <div className="min-w-[300px] lg:flex flex-col p-4 shadow-md rounded-lg h-fit hidden">
          ...
          <div
            onClick={multiFilter}
            className="flex justify-center items-center py-2 bg-[#276968] text-white rounded-lg cursor-pointer"
          >
            <h1>Filter</h1>
          </div>
        </div>
